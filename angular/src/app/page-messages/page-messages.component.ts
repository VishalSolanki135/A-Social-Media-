...
export class PageMessagesComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    let userDataEvent = this.events.getUserData.subscribe((user)=>{
      ...
      this.activeMessage.fromId = this.activeMessage.fromId ||
      user.messages[0].from_id;
      ...
      this.setActiveMessages(this.activeMessage.fromId);
    });
  ...
  public deleteMessage(msgId) {
    ...
    let requestObject = {
      location: `users/delete-messages/${msgId}`,
      type: "POST"
    }
      ...
      if(val.statusCode == 201) {
        for(let i = 0; i < this.messages.length; i++) {
          if(this.messages[i]._id == msgId) {
            ...
            this.setActiveMessages(this.messages[0].from_id);
            break;
          }
        }
      }
